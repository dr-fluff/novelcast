...
const UnifiedPanel = (() => {
    ...
    function renderTagList(containerId, panelId, values) {
        ...
        list.forEach((value) => {
            ...
            tag.innerHTML =
                value +
                ' <button type="button" class="tag-remove" aria-label="Remove"><i class="fa-solid fa-xmark"></i></button>';
        ...
        async confirm(panelId) {
            ...
            try {
                const payload = {
                    ...
                    selected_chapters: state.selectedChapters.length > 0 ? state.selectedChapters : null,
                };
    ...
    handlers.metadata = {
        ...
        async loadData(panelId) {
            ...
            try {
                const result = await fetchJSON(`/api/stories/${this.storyId}`);
                if (result.ok) {
                    ...
                    if (hideAuthorNotesEl) hideAuthorNotesEl.checked = Boolean(st.hide_author_notes);

                    const autoUpdateEl = $(`metaAutoUpdate-${panelId}`);
                    if (autoUpdateEl) autoUpdateEl.checked = Boolean(st.auto_update);

                    this.renderCoverPreview(panelId, st.cover_path);

                    renderTagList('metaSeriesWrap', panelId, st.series_list || parseCommaList(st.series));
                    renderTagList('metaGenresWrap', panelId, st.genres_list || parseCommaList(st.genres));
                    renderTagList('metaTagsWrap', panelId, st.tags_list || parseCommaList(st.tags));
                }
            } catch (_) {}
        ...
        async save(panelId) {
            ...
            try {
                ...
                const result = await fetchJSON(`/api/stories/${this.storyId}/metadata`, {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
                });
        ...
        async handleCoverFileSelect(panelId, inputEl) {
            ...
            if (statusEl) {
                ...
                statusEl.className = 'unified-panel-status';
            }
            ...
            try {
                ...
                if (statusEl) {
                    ...
                    statusEl.className = 'unified-panel-status error';
                }
            } finally {
                inputEl.value = '';
            }
    ...
    handlers.author = {
        ...
        storyId: null,
        authorId: null,
        _pendingPayload: null, // payload from the save attempt that hit a 409
        ...
        open(panelId, storyId, authorId) {
            ...
            setStatus(panelId, '', '');
            this.clearConflictPrompt(panelId);
            setOpen(panelId, true);
            this.loadData(panelId);
        },
        ...
        async save(panelId) {
            ...
            try {
                const payload = this._buildPayload(panelId);

                const result = await fetchJSON(this._saveUrl(), {
                    method: 'PATCH',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(payload),
                });
                ...
                if (!result.ok) {
                    if (result.status === 409) {
                        ...
                        if (conflict) {
                            ...
                            setStatus(panelId, '', '');
                            this.showConflictPrompt(panelId, conflict);
                            return;
                        }
                    }
                    console.log(result);
        ...
        showConflictPrompt(panelId, conflict) {
            const statusEl = $(`status-${panelId}`);
            if (!statusEl) return;
            ...
            box.id = `authorConflictPrompt-${panelId}`;
            box.className = 'unified-panel-conflict';

            const msg = document.createElement('p');
            msg.textContent = `An author named "${conflict.name}" already exists. Merge into them, or save this as a separate author anyway?`;
            box.appendChild(msg);
            ...
            mergeBtn.onclick = () => this.confirmMerge(panelId, conflict.id);

            const forceBtn = document.createElement('button');
            forceBtn.type = 'button';
            forceBtn.className = 'btn btn-secondary';
            forceBtn.textContent = 'Save as separate author';
            forceBtn.onclick = () => this.forceSave(panelId);

            const cancelBtn = document.createElement('button');
            cancelBtn.type = 'button';
            cancelBtn.className = 'btn btn-link';
            cancelBtn.textContent = 'Cancel';
            cancelBtn.onclick = () => this.clearConflictPrompt(panelId);

            btnRow.append(mergeBtn, forceBtn, cancelBtn);
            box.appendChild(btnRow);

            statusEl.insertAdjacentElement('afterend', box);
        },

        clearConflictPrompt(panelId) {
            const existing = $(`authorConflictPrompt-${panelId}`);
            if (existing) existing.remove();
        },

        async confirmMerge(panelId, primaryAuthorId) {
            const saveBtn = $(`authorSaveBtn-${panelId}`);
            if (saveBtn) saveBtn.disabled = true;
            this.clearConflictPrompt(panelId);
            setStatus(panelId, 'Merging…', '');
            ...
            try {
                ...
                if (!result.ok) {
                    throw new Error(result.data.detail || 'Failed to merge');
                }
        ...
        async forceSave(panelId) {
            ...
            try {
                const payload = this._pendingPayload || this._buildPayload(panelId);
